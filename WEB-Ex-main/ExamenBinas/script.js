...
function showResult() {
    ...
    if(userScore === 30){
        ...
        scoreTag = '<span> y  Muy bueno 😎, Tienes <p>' + userScore + '</p> de <p>' + questions.length + '</p></span>';
    } else { 
...
function startTimer(time){
    ...
    function timer(){
        ...
        if(time < 0){ //if timer is less than 0
            ...
            // Auto mark the question as incorrect
            for(i=0; i < allOptions; i++){
                ...
                if(option_list.children[i].textContent == correcAns){ //if there is an option which is matched to an array answer
                    ...
                    option_list.children[i].setAttribute("class", "option incorrect"); //adding red color to unmatched option
                    option_list.children[i].insertAdjacentHTML("beforeend", crossIconTag); //adding cross icon to unmatched option
                }
            }

            console.log("Time Off: Auto marked as incorrect.");
